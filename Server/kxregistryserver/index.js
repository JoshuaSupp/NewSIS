...
app.post("/api/post", (req,res)=>{
    ...
    const sqlInsert = "INSERT INTO kx_registry (index_no,full_name,age,school,parent_name,parent_contact,parent_email,comments) VALUES (?,?,?,?,?,?,?,?)";
...
app.put('/updatereg/:id', (req, res) => {
  const sqlGet = "UPDATE kx_registry SET `full_name` = ?, `age` = ?, `school` = ?, `parent_name` = ?, `parent_contact` = ?, `parent_email` = ? , `comments` = ?  WHERE id = ?"
  const id = req.params.id;
