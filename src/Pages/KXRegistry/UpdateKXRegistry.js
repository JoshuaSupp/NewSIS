...
function UpdateKXRegistry(){
    ...
    useEffect(()=> {
        axios.get('http://localhost:5001/editreg/'+id)
        .then(res => {
    ...
    const handleSubmit = (e) =>{
        e.preventDefault();
        axios.put('http://localhost:5001/updatereg/'+id, {full_name,age,school,parent_name,parent_contact,parent_email,comments})
        .then(res => {
            if(res.data.updated){
                navigate('../KXRegistry')
            }else{
 ...
 <button class="button button1"><a href="/KXRegistry">Back To KX0001 Registry</a></button>
    <h1>Update KX Student Registry</h1>
...
export default UpdateKXRegistry