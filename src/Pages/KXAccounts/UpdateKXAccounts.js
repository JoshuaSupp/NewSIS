...
const UpdateKXAccounts = () => {
    ...
    useEffect(()=> {
        axios.get('http://localhost:5004/editkxs/'+id)
        .then(res => {
    ...
    const handleSubmit = (e) =>{
       ...
       if (name === 'password' && value.length > 10) {
        return; 
          ...
          axios.put('http://localhost:5004/updatekxs/'+id, {student_name,username,password})
          .then(res => {
              if(res.data.updated){
                  navigate('../ANAccounts')
              }else{
  ...
  return (
    <div class='addadmin'>
    <Navbar/>
  <h1>Update KX Student Account</h1>
      <br />
...
export default UpdateKXAccounts